...
void main()
{
    ...
    // 抜刀中hand.Rへ受け渡し、以手首ボーンへ完全追従
   
    void DrawBattleSword(const glm::mat4& handWorldTransform,
        const glm::mat4& sheathWorldTransform, bool attacking, float attackProgress)
    {
        
        glm::mat4 sword = sheathWorldTransform;
        float gripOffset = 0.0f;
        if (attacking) {
            attackProgress = glm::clamp(attackProgress, 0.0f, 1.0f);
            if (attackProgress >= 0.78f) {
              
                // 消えたり、鞘へ瞬間移動したりしないように
                float sheathT = glm::clamp(
                    (attackProgress - 0.78f) / 0.16f, 0.0f, 1.0f);
                sheathT = sheathT * sheathT * (3.0f - 2.0f * sheathT);
                glm::vec3 handPosition(handWorldTransform[3]);
                glm::vec3 sheathPosition(sheathWorldTransform[3]);
                glm::quat handRotation = glm::normalize(
                    glm::quat_cast(glm::mat3(handWorldTransform)));
                glm::quat sheathRotation = glm::normalize(
                    glm::quat_cast(glm::mat3(sheathWorldTransform)));
                glm::vec3 sheathingPosition = glm::mix(
                    handPosition, sheathPosition, sheathT);
                glm::quat sheathingRotation = glm::slerp(
                    handRotation, sheathRotation, sheathT);
                sword = glm::translate(glm::mat4(1.0f), sheathingPosition) *
                    glm::mat4_cast(sheathingRotation);
                gripOffset = glm::mix(0.40f, 0.0f, sheathT);
            }
            else if (attackProgress >= 0.065f) {
                
                float drawT = glm::clamp((attackProgress - 0.065f) / 0.065f, 0.0f, 1.0f);
                drawT = drawT * drawT * (3.0f - 2.0f * drawT);
                glm::vec3 sheathPosition(sheathWorldTransform[3]);
                glm::vec3 handPosition(handWorldTransform[3]);
                glm::quat sheathRotation = glm::normalize(
                    glm::quat_cast(glm::mat3(sheathWorldTransform)));
                glm::quat handRotation = glm::normalize(
                    glm::quat_cast(glm::mat3(handWorldTransform)));
                glm::vec3 drawnPosition = glm::mix(sheathPosition, handPosition, drawT);
                glm::quat drawnRotation = glm::slerp(sheathRotation, handRotation, drawT);
                sword = glm::translate(glm::mat4(1.0f), drawnPosition) *
                    glm::mat4_cast(drawnRotation);
                
                gripOffset = glm::mix(0.0f, 0.40f, drawT);
            }
        }
    
        sword = glm::translate(sword, glm::vec3(0.0f, gripOffset, 0.0f));
        
        sword = glm::rotate(sword, glm::radians(7.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        sword = glm::rotate(sword, glm::radians(-5.0f), glm::vec3(1.0f, 0.0f, 0.0f));

        
        glm::mat4 blade = glm::translate(sword, glm::vec3(0.0f, 0.13f, 0.0f));
        blade = glm::scale(blade, glm::vec3(0.13f, 1.48f, 0.065f));
        DrawSwordPart(blade, glm::vec3(0.66f, 0.78f, 0.88f), 0.88f, 0.20f,
            attacking ? glm::vec3(0.025f, 0.055f, 0.08f) : glm::vec3(0.0f));

        glm::mat4 guard = glm::translate(sword, glm::vec3(0.0f, 0.025f, 0.0f));
        guard = glm::scale(guard, glm::vec3(0.62f, 0.105f, 0.13f));
        DrawSwordPart(guard, glm::vec3(0.30f, 0.20f, 0.48f), 0.72f, 0.28f);

        glm::mat4 grip = glm::translate(sword, glm::vec3(0.0f, -0.40f, 0.0f));
        grip = glm::scale(grip, glm::vec3(0.15f, 0.43f, 0.15f));
        DrawSwordPart(grip, glm::vec3(0.075f, 0.055f, 0.10f), 0.18f, 0.72f);

    
        glm::mat4 pommel = glm::translate(sword, glm::vec3(0.0f, -0.51f, 0.0f));
        pommel = glm::scale(pommel, glm::vec3(0.22f, 0.12f, 0.22f));
        DrawSwordPart(pommel, glm::vec3(0.34f, 0.23f, 0.50f), 0.76f, 0.25f);
...
const ImportedModel* GetSharedEnemyModel(int index)
{
    ...
    if (g_enemyModels[index].isLoaded()) return &g_enemyModels[index];
    if (g_enemyModels[0].isLoaded()) return &g_enemyModels[0];
    return nullptr;
}
...
{
    LoadBattleEnemyModels();
    for (ImportedModel& model : g_enemyModels) {
        if (model.isLoaded()) model.updateAnimation(deltaTime);
    }
}
...
void CombatSystem::renderBattleScene(Character* activeChar, int screenWidth, int screenHeight)
{
    ...
    constexpr float attackPlaybackRate = 1.0f;
    constexpr float attackHitTime = 53.0f / 30.0f;
    constexpr float attackDuration = 110.0f / 30.0f;
    constexpr float enemyAttackHitTime = 0.62f;
    constexpr float enemyAttackDuration = 1.28f;
    if (playerCommandAnimating) {
 
        g_playerModel.updateAnimation(animationDelta * attackPlaybackRate);
        ...
        if (playerCommandAnimationTime >= attackDuration) {
            ...
            if (tutorialStep == TutorialStep::WaitingForAttack) {
                tutorialStep = TutorialStep::Skill;
            }
        }
    }
    else {
    ...
    // 接近モーションとダメージを同期
    if (enemyCommandAnimating) {
        enemyCommandAnimationTime += animationDelta;
        if (!enemyCommandHitApplied &&
            enemyCommandAnimationTime >= enemyAttackHitTime) {
            enemyCommandHitApplied = true;
            executeSkill(queuedEnemyAttacker, queuedEnemyTarget);
        }
        if (enemyCommandAnimationTime >= enemyAttackDuration) {
    ...
    const auto getEnemyBattlePosition = [enemyCount](int enemyIndex) {
        const float spacing = 2.65f;
        const float side = enemyIndex - (enemyCount - 1) * 0.5f;
        const float depth = -3.55f + side * 0.34f;
        return glm::vec3(side * spacing + 1.35f, 0.0f, depth);
    };
    ...
    for (auto* character : participants) {
        ...
        if (character->isAlly == 1) {
            const glm::vec3 idlePosition(-2.45f, 0.0f, 8.0f);
            position = idlePosition;

            //カメラを動かさずplayerをを敵へ移動
            // 敵の手前で止め、モデル同士の貫通を防止
            if (playerCommandAnimating && character == queuedPlayerAttacker &&
                queuedPlayerTarget) {
                auto targetSlot = std::find(
                    fixedEnemyOrder.begin(), fixedEnemyOrder.end(), queuedPlayerTarget);
                if (targetSlot != fixedEnemyOrder.end()) {
                    const int targetIndex = static_cast<int>(
                        std::distance(fixedEnemyOrder.begin(), targetSlot));
                    const glm::vec3 targetPosition = getEnemyBattlePosition(targetIndex);
                    glm::vec3 approachDirection = idlePosition - targetPosition;
                    approachDirection.y = 0.0f;
                    if (glm::length(approachDirection) > 0.001f)
                        approachDirection = glm::normalize(approachDirection);

                    const glm::vec3 strikePosition =
                        targetPosition + approachDirection * 2.2f;
                    const float progress = glm::clamp(
                        playerCommandAnimationTime / attackDuration, 0.0f, 1.0f);

                    // smoothstepを区間ごとに
                    float travel = 0.0f;
                    if (progress >= 0.18f && progress < 0.46f) {
                        const float t = glm::clamp(
                            (progress - 0.18f) / 0.28f, 0.0f, 1.0f);
                        travel = t * t * (3.0f - 2.0f * t);
                    }
                    else if (progress >= 0.46f && progress < 0.70f) {
                        travel = 1.0f;
                    }
                    else if (progress >= 0.70f && progress < 0.96f) {
                        const float t = glm::clamp(
                            (progress - 0.70f) / 0.26f, 0.0f, 1.0f);
                        travel = 1.0f - t * t * (3.0f - 2.0f * t);
                    }
                    position = glm::mix(idlePosition, strikePosition, travel);
                }
            }
            ...
            position = getEnemyBattlePosition(enemyIndex);
            const glm::vec3 enemyIdlePosition = position;

            // 行動中の敵だけをプレイヤーの近くへ移動
            if (enemyCommandAnimating && character == queuedEnemyAttacker &&
                queuedEnemyTarget) {
                const glm::vec3 playerPosition(-2.45f, 0.0f, 8.0f);
                glm::vec3 approachDirection = enemyIdlePosition - playerPosition;
                approachDirection.y = 0.0f;
                if (glm::length(approachDirection) > 0.001f)
                    approachDirection = glm::normalize(approachDirection);
                const glm::vec3 strikePosition =
                    playerPosition + approachDirection * 2.2f;
                const float progress = glm::clamp(
                    enemyCommandAnimationTime / enemyAttackDuration, 0.0f, 1.0f);

                float travel = 0.0f;
                if (progress >= 0.12f && progress < 0.42f) {
                    const float t = glm::clamp(
                        (progress - 0.12f) / 0.30f, 0.0f, 1.0f);
                    travel = t * t * (3.0f - 2.0f * t);
                }
                else if (progress >= 0.42f && progress < 0.64f) {
                    travel = 1.0f;
                }
                else if (progress >= 0.64f && progress < 0.96f) {
                    const float t = glm::clamp(
                        (progress - 0.64f) / 0.32f, 0.0f, 1.0f);
                    travel = 1.0f - t * t * (3.0f - 2.0f * t);
                }
                position = glm::mix(enemyIdlePosition, strikePosition, travel);
            }
        ...
        if (character->isAlly == 0) {
            ...
            glm::mat4 handModelTransform(1.0f);
            glm::mat4 handWorldTransform = glm::translate(
                glm::mat4(1.0f), position + glm::vec3(0.72f, 2.02f, 0.0f));
            if (g_playerModel.getBonePoseTransform("hand.R", handModelTransform)) {
                // actorの拡大は手の位置だけへ適用し、武器自体の寸法には適用しない。
                glm::vec3 handWorldPosition = glm::vec3(
                    model * glm::vec4(glm::vec3(handModelTransform[3]), 1.0f));
                handModelTransform[3] = glm::vec4(handWorldPosition, 1.0f);
                handWorldTransform = handModelTransform;
            }
            const bool swordAttacking =
                playerCommandAnimating && character == queuedPlayerAttacker;
            const float swordAttackProgress = swordAttacking
                ? glm::clamp(playerCommandAnimationTime / attackDuration, 0.0f, 1.0f)
                : 0.0f;
            // 鞘は左腰の後方へ固定する。待機中はここに剣を表示し、
            // 攻撃開始後にDrawBattleSword内で右手へ受け渡す。
            // 固定ワールド座標では、モデル拡大率や姿勢によって腰から外れる。
            // 左大腿ボーンの付け根を実際の左腰として、描画モデル行列で
            // ワールド位置へ変換してから小さな外側オフセットだけを加える。
            glm::vec3 sheathPosition = position + glm::vec3(-0.46f, 1.78f, 0.10f);
            glm::mat4 leftHipModelTransform(1.0f);
            if (g_playerModel.getBonePoseTransform("uperleg.L", leftHipModelTransform)) {
                sheathPosition = glm::vec3(
                    model * glm::vec4(glm::vec3(leftHipModelTransform[3]), 1.0f));
                sheathPosition += glm::vec3(-0.16f, 0.10f, 0.12f);
            }
            glm::mat4 sheathWorldTransform = glm::translate(
                glm::mat4(1.0f), sheathPosition);
            // 剣モデルの+Yが刃先方向。左腰では鍔を上に置き、刃先が後方の
            // 斜め下へ伸びるよう約180度反転してから15度傾ける。
            sheathWorldTransform = glm::rotate(sheathWorldTransform,
                glm::radians(165.0f), glm::vec3(0.0f, 0.0f, 1.0f));
            sheathWorldTransform = glm::rotate(sheathWorldTransform,
                glm::radians(8.0f), glm::vec3(1.0f, 0.0f, 0.0f));
            DrawBattleSword(handWorldTransform, sheathWorldTransform,
                swordAttacking, swordAttackProgress);
        }
    }
    glBindVertexArray(0);
    ...
    std::vector<ImVec4> occupiedEnemyHudRects;
    for (size_t i = 0; i < enemyPositions.size(); ++i) {
        ...
        std::string enemyLabel = enemy->isBroken
            ? std::string(u8"耐性崩壊") + "  /  Lv. " + std::to_string(enemy->level)
            : enemy->name + "  [ " + enemy->affinity + " ]  Lv. " + std::to_string(enemy->level);
        const ImVec2 labelSize = ImGui::CalcTextSize(enemyLabel.c_str());
        const float hudHalfWidth = std::max(58.0f, labelSize.x * 0.5f);
        float barY = head.y - 62.0f;

        // 画面上で敵同士が近い場合は、後のHUDを上段へ逃がす。
        for (int attempt = 0; attempt < 8; ++attempt) {
            const ImVec4 candidate(head.x - hudHalfWidth, barY - 24.0f,
                                   head.x + hudHalfWidth, barY + 12.0f);
            bool overlaps = false;
            for (const ImVec4& occupied : occupiedEnemyHudRects) {
                if (candidate.x < occupied.z + 8.0f && candidate.z + 8.0f > occupied.x &&
                    candidate.y < occupied.w + 6.0f && candidate.w + 6.0f > occupied.y) {
                    overlaps = true;
                    break;
                }
            }
            if (!overlaps) {
                occupiedEnemyHudRects.push_back(candidate);
                break;
            }
            barY -= 42.0f;
        }

        ImVec2 barMin(head.x - 58.0f, barY);
        ImVec2 barMax(head.x + 58.0f, barY + 14.0f);
        drawList->AddText(ImGui::GetFont(), 15.0f,
            ImVec2(head.x - labelSize.x * 0.5f, barY - 22.0f),
        ...
        if (enemy->maxTideguard > 0) {
            ...
            char resistanceText[32];
            std::snprintf(resistanceText, sizeof(resistanceText), u8"耐性 %d / %d",
                enemy->tideguard, enemy->maxTideguard);
            drawList->AddText(ImGui::GetFont(), 12.0f,
                ImVec2(guardMin.x, guardMax.y + 2.0f),
                IM_COL32(151, 230, 226, 235), resistanceText);
        }
...
void CombatSystem::renderActionMenu(Character* activeChar, int screenWidth, int screenHeight)
{
    ...
    if (ImGui::Begin("##PartyHud", nullptr, hudFlags)) {
        ...
        for (auto* c : participants) {
            ...
            float apFraction = maxPoints > 0 ? (float)currentPoints / (float)maxPoints : 0.0f;
            hpFraction = std::max(0.0f, std::min(hpFraction, 1.0f));
            apFraction = std::max(0.0f, std::min(apFraction, 1.0f));
            ...
            dl->AddText(hudFont, nameFontSize,
                ...
                c->name.c_str());
            char levelText[24];
            snprintf(levelText, sizeof(levelText), "Lv. %d", c->level);
            ImVec2 levelSize = hudFont->CalcTextSizeA(17.0f, FLT_MAX, 0.0f, levelText);
            dl->AddText(hudFont, 17.0f,
                ImVec2(origin.x + 449.0f - levelSize.x, origin.y + rowY + 12.0f),
                IM_COL32(224, 195, 255, 245), levelText);
            dl->AddLine(
            ...
            snprintf(apText, sizeof(apText), "%d / %d", currentPoints, maxPoints);

            dl->AddText(hudFont, statFontSize,
                ImVec2(textX, origin.y + rowY + 38.0f), IM_COL32(246, 240, 252, 255), "HP");
            dl->AddText(hudFont, statFontSize,
                ImVec2(textX, origin.y + rowY + 76.0f), IM_COL32(213, 182, 250, 255), "AP");
            ImVec2 hpValueSize = hudFont->CalcTextSizeA(statFontSize, FLT_MAX, 0.0f, hpText);
            ImVec2 apValueSize = hudFont->CalcTextSizeA(statFontSize, FLT_MAX, 0.0f, apText);
            dl->AddText(hudFont, statFontSize,
                ImVec2(origin.x + 449.0f - hpValueSize.x, origin.y + rowY + 38.0f),
                IM_COL32(250, 246, 255, 255), hpText);
            dl->AddText(hudFont, statFontSize,
                ImVec2(origin.x + 449.0f - apValueSize.x, origin.y + rowY + 76.0f),
                IM_COL32(226, 202, 255, 245), apText);

            // HPとAP描画
            const float hpY = origin.y + rowY + 60.0f;
            const float apY = origin.y + rowY + 98.0f;
            dl->AddRectFilled(ImVec2(barX, hpY), ImVec2(barX + barWidth, hpY + 6.0f),
                IM_COL32(24, 24, 39, 230), 2.0f);
            dl->AddRectFilled(ImVec2(barX, apY), ImVec2(barX + barWidth, apY + 5.0f),
            ...
            dl->AddRectFilled(ImVec2(barX, hpY), ImVec2(barX + barWidth * hpFraction, hpY + 6.0f),
                hpColor, 2.0f);
            dl->AddRectFilled(ImVec2(barX, apY), ImVec2(barX + barWidth * apFraction, apY + 5.0f),
                IM_COL32(180, 119, 241, 255), 2.0f);
            dl->AddCircleFilled(ImVec2(barX + barWidth * hpFraction, hpY + 3.0f), 3.5f, hpColor, 10);
            dl->AddCircleFilled(ImVec2(barX + barWidth * apFraction, apY + 2.5f), 3.0f,
    ...
    if (ImGui::Begin("##CommandHud", nullptr, hudFlags)) {
        ...
        // 味方・敵ターンでパネル位置を変えない
            ...
            const TutorialStep tutorialStepAtFrameStart = tutorialStep;
            const bool tutorialGuiding =
                tutorialStepAtFrameStart != TutorialStep::None &&
                tutorialStepAtFrameStart != TutorialStep::Complete;
            const bool enabled[] = {
                !tutorialGuiding || tutorialStepAtFrameStart == TutorialStep::Attack,
                currentPoints >= 1 &&
                    (!tutorialGuiding || tutorialStepAtFrameStart == TutorialStep::Skill),
                (attackItemCount > 0 || healingItemCount > 0) && !tutorialGuiding,
                !tutorialGuiding,
                false
            };
            ...
            for (int i = 0; i < 5; ++i) {
                ...
                if (i == 0) {
                    ...
                    if (tutorialStep == TutorialStep::Skill) {
                    ...
                    ImGui::OpenPopup("##BattleItemPopup");
                }
                else if (i == 3) {
            ...
            if (ImGui::BeginPopup("##BattleItemPopup")) {
                ImGui::TextUnformatted(u8"使用するアイテム");
                ImGui::Separator();
                const std::string attackLabel = std::string(u8"攻撃強化アンプル  x") +
                    std::to_string(attackItemCount);
                if (attackItemCount > 0 && ImGui::Button(attackLabel.c_str(), ImVec2(250.0f, 38.0f))) {
                    if (useAttackItem()) advanceTurn(activeChar);
                    ImGui::CloseCurrentPopup();
                }
                const std::string healingLabel = std::string(u8"回復キット  x") +
                    std::to_string(healingItemCount);
                if (healingItemCount > 0 && ImGui::Button(healingLabel.c_str(), ImVec2(250.0f, 38.0f))) {
                    if (useHealingItem()) advanceTurn(activeChar);
                    ImGui::CloseCurrentPopup();
                }
                ImGui::EndPopup();
            }
            ...
            // プレイヤーの攻撃モーションが終わるまで敵側の攻撃を開始しない
            if (!playerCommandAnimating &&
                !enemyActionQueued && !enemyCommandAnimating) {
                enemyActionQueued = true;
                enemyActionTime = ImGui::GetTime() + 0.6;
            }

            if (!playerCommandAnimating && enemyActionQueued &&
                !enemyCommandAnimating && ImGui::GetTime() >= enemyActionTime) {
                Character* target = getRandomAliveTarget(1);
                if (target) {
                    enemyCommandAnimating = true;
                    enemyCommandHitApplied = false;
                    enemyCommandAnimationTime = 0.0f;
                    queuedEnemyAttacker = activeChar;
                    queuedEnemyTarget = target;
                }
            }
        }

        const bool tutorialVisible =
            tutorialStep != TutorialStep::None &&
            tutorialStep != TutorialStep::Complete;
        if (tutorialVisible) {
            ImDrawList* guide = ImGui::GetForegroundDrawList();
            guide->AddRectFilled(
                ImVec2(0.0f, 0.0f), ImVec2((float)screenWidth, (float)screenHeight),
                IM_COL32(2, 5, 14, 112));

            const bool playerCanChoose = activeChar && activeChar->isAlly == 1;
            if (playerCanChoose &&
                (tutorialStep == TutorialStep::Attack || tutorialStep == TutorialStep::Skill)) {
                const ImVec2 commandOrigin = ImGui::GetWindowPos();
                const int row = tutorialStep == TutorialStep::Attack ? 0 : 1;
                const float rowY = 62.0f + 68.0f * (float)row;
                const ImVec2 highlightMin(commandOrigin.x + 48.0f, commandOrigin.y + rowY - 4.0f);
                const ImVec2 highlightMax(commandOrigin.x + 360.0f, commandOrigin.y + rowY + 62.0f);
                const float pulse = 0.5f + 0.5f * std::sin((float)ImGui::GetTime() * 4.0f);
                guide->AddRect(highlightMin, highlightMax,
                    IM_COL32(230, 190, 255, (int)(190.0f + pulse * 65.0f)),
                    7.0f, 0, 3.0f);
            }

            const float panelWidth = std::min(620.0f, (float)screenWidth - 80.0f);
            const ImVec2 panelMin(((float)screenWidth - panelWidth) * 0.5f, 54.0f);
            const ImVec2 panelMax(panelMin.x + panelWidth, panelMin.y + 112.0f);
            guide->AddRectFilled(panelMin, panelMax, IM_COL32(9, 15, 31, 242), 8.0f);
            guide->AddRect(panelMin, panelMax, IM_COL32(177, 129, 235, 230), 8.0f, 0, 1.5f);
            guide->AddText(ImVec2(panelMin.x + 22.0f, panelMin.y + 16.0f),
                IM_COL32(218, 190, 255, 255), u8"戦闘チュートリアル");

            const char* guideText = u8"敵を攻撃して耐性ゲージを削りましょう。クリックで開始します。";
            if (tutorialStep == TutorialStep::Attack) {
                guideText = u8"「攻撃」を選択してください。通常攻撃を使うとAPを1獲得します。";
            }
            else if (tutorialStep == TutorialStep::Skill) {
                guideText = playerCanChoose
                    ? u8"APを消費して「スキル」を使ってみましょう。"
                    : u8"APを獲得しました。";
            }
            guide->AddText(ImVec2(panelMin.x + 22.0f, panelMin.y + 52.0f),
                IM_COL32(243, 240, 250, 255), guideText);

            if (tutorialStep == TutorialStep::Intro &&
                (ImGui::IsMouseClicked(ImGuiMouseButton_Left) || ImGui::IsKeyPressed(ImGuiKey_Enter))) {
                tutorialStep = TutorialStep::Attack;
            }
        }
    }
    ImGui::End();

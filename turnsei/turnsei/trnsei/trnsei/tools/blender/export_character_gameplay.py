...
# フィールド側がキャラクター原点を移動させるため、歩行クリップ内の
# Hips水平移動を残すと移動が二重になり、接地足が後方へ滑って見える。
# 先頭から末尾までのドリフトだけを各フレームから差し引き、上下動と
# 左右の重心移動は保持したインプレース歩行へ変換する。
hips = channels.get("Hips")
if hips and len(hips["position"]) > 1:
    positions = hips["position"]
    first = positions[0][:]
    last = positions[-1][:]
    for frame, position in enumerate(positions):
        t = frame / (len(positions) - 1)
        # Blender座標のX/Yを水平成分として扱い、Zの腰上下動は残す。
        position[0] -= (last[0] - first[0]) * t
        position[1] -= (last[1] - first[1]) * t

    # ループ境界の微小な差をなくす。最後の姿勢を最初と同一にすると、
    # fmodで0Fへ戻った瞬間の腰・足首のポップを防げる。
    for channel in channels.values():
        channel["position"][-1] = channel["position"][0][:]
        channel["rotation"][-1] = channel["rotation"][0][:]
        channel["scale"][-1] = channel["scale"][0][:]
